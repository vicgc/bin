...
For usage and help:
    ...
    contacts.py -h              # Brief usage and list of options
    contacts.py --full-help     # Full help including examples and notes
"""
    ...
    def event_attributes(self, info):
        ...
        for line in info:
            ...
            if mo:
                ...
                if em or ph or item_key == 'remind':
        ...
        if 'when' in attributes:
            ...
            if 'remind' in attributes:
                ...
                for reminder in attributes['remind']:
                    LOG.debug("reminder: {var}".format(var=reminder))
                    ...
                    if match:
                        ...
                        LOG.error("Reminder: {var}. Invalid format.".format(var=reminder))
                LOG.debug("reminders: {var}".format(var=reminders))
...
EVENT ATTRIBUTES:
    ...
    remind
        ...
        Notes: The google calendar account has to be configured for
            some options, eg sms, to work. See calendar settings on
            google page.

    Each attribute should appear at most one time in a calendar event.
...
CONFIGURATION:
    ...
    The contacts.py script can be used as an address book for mutt.
    ...
    ~/.mutt/bindings
        ...
        <pipe-message>/path/to/contacts.py -c '-'<return>\\
        <enter-command>set wait_key=$my_wait_key<enter>'

    # Set the query_command to use contacts.py to search gmail contacts
    # for email addresses. Note: mutt requires a status header line so use
    # the include-header, -r, option.

    ~/.mutt/muttrc
        set query_command = "/root/dm/bin/contacts.py -m short -r -s name %s"
